...
| 3 | 影響範囲・依存関係調査 | タスク一覧、実行計画、既存資産 | 影響範囲表、更新要否、依存・競合、リスク、調査状態 | docs、Skill、Agent、コード、テスト、設定、生成物、CI/CD、外部サービスを調査し、`confirmed／未確認／対象外`を記録している |
...
| 7 | docs作成 | 確定Issue、影響範囲表、専用worktree | 仕様・設計・利用方法等のdocs | Issueの完了条件と実装方針が日本語docsに反映されている |
| 8 | docsレビュー | docs、Issue、影響範囲表 | docsレビューコメント、修正版docs | 🔴がない。Issueとの矛盾と影響範囲の更新漏れがない |
