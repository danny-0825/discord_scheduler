...
| 2 | 実行計画 | タスク一覧、関係表 | タスクグラフ、SubAgent割当、Issue/Agent/branch/worktree/PR対応表 | read-only | 独立タスクと依存タスクの実行順、scope、担当、検証担当が確定している | Planを確定し、未定義のTask・担当・scopeを0件にする |
...
| 5 | Issueレビュー | Issue、関係表、影響範囲表 | Issueレビューコメント、修正版Issue、関係性レビュー結果 | GitHubのみ | 🔴がない。Issue間の依存・競合・SubAgent割当・影響範囲が追跡可能である | 🔴0件、Planのpre-branch項目completed、write開始禁止を解除 |
| 6 | 基点更新・worktree/branch作成 | 確定Issue、Git状態 | 最新の基点から作成した専用worktreeと`feature/{IssueNo}-{short-description}` | write開始ゲート | `git fetch`後に最新基点を確認し、タスク専用環境が作成されている | 基点SHA、branch、worktree、write scopeをPlanへ記録 |
